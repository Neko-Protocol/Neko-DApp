...
const Swap: React.FC = () => {
  ...
  // Get available tokens for current network (dynamic - updates when network changes)
  const availableTokens = getAvailableTokens();
  const tokenCodes = Object.keys(availableTokens);
  const tokens = getTokens(); // Get current network tokens
  ...
  useEffect(() => {
    const newAvailableTokens = getAvailableTokens();
    const newTokenCodes = Object.keys(newAvailableTokens);
    const newTokens = getTokens();

    // If current tokens are not in the new network's available tokens, reset to defaults
    const tokenInId = typeof tokenIn === "string" ? tokenIn : tokenIn.contract || "";
    const tokenOutId = typeof tokenOut === "string" ? tokenOut : tokenOut.contract || "";

    const isTokenInValid = Object.values(newAvailableTokens).some(
      (token) => token.contract === tokenInId
    );
    const isTokenOutValid = Object.values(newAvailableTokens).some(
      (token) => token.contract === tokenOutId
    );

    if (!isTokenInValid || !isTokenOutValid) {
      setTokenIn(newAvailableTokens[newTokenCodes[0]]?.contract || newTokens.XLM || "");
      setTokenOut(newAvailableTokens[newTokenCodes[1]]?.contract || newTokens.USDC || "");
    }
  ...
  const fetchLiveQuote = useCallback(async () => {
    ...
    try {
      const quoteRequest: QuoteRequest = {
        ...
        tradeType: "EXACT_IN",
        // Use multiple protocols to find routes (SOROSWAP, AQUA, SDEX)
        protocols: ["soroswap", "aqua"],
        slippageBps: 100, // 1% slippage
        maxHops: 3, // Allow intermediate hops for routing
      };
